/**
 * M8 可观测性 Mock 适配器（trace / 日志 / 续跑点）。
 *
 * 暴露：getRun(runId)
 * 字段对齐 demo types/observability.ts 的 ObservabilityRun。
 *
 * 设计依据：01-api-spec.md §8。
 */
import { clone, delay, ok } from './util'

/** 可观测性运行记录（trace / 日志 / 续跑点） */
const OBSERVABILITY_RUNS = [
  {
    run_id: 'run-002',
...
/** 按 run_id 读取可观测性运行记录（缺省返回空 trace） */
export async function getRun(runId) {
  await delay()
  const found = OBSERVABILITY_RUNS.find((r) => r.run_id === runId)
  if (found) return ok(clone(found))
  return ok({
    run_id: runId,
    agent_name: '',
    trace: [],
