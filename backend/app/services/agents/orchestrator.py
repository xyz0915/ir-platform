...
class Orchestrator:
    ...
    async def dispatch(
        ...
        self._state_machine(run_id, result=result)
        return result
    ...
    def _state_machine(
        ...
        - 否则 → status=completed（本期骨架仅做单步串行；多步在 A 批次扩展）
        - confidence / result_json 回写
        """
        if failed:
            AgentRun.update(run_id, status=self.STATUS_FAILED)
            return
        update_kwargs: dict[str, Any] = {"status": self.STATUS_COMPLETED}
    ...
    async def run_pipeline(
        ...
        # 2) 调查（含防御式调用 RootCauseAgent）
        await self.dispatch(run_id, InvestigatorAgent(), ctx, task={})
        # 3) 处置（默认零自主 → 触发 HITL 网关，run 变为 waiting_hitl）
        await self.dispatch(run_id, ResponderAgent(), ctx, task={})
    ...
    async def resume(
        ...
        if approval.get("status") == HitlApproval.STATUS_APPROVED:
            ...
            try:
                target = _json.loads(approval.get("target_json") or "{}")
    ...
    async def _finish_with_reporter(
        ...
        result = await self.dispatch(run_id, reporter, ctx, task=task)
